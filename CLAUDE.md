...
python3 tools/build-about.py                 # rebuild the /about timeline photos
...
```
index.php  about.php  gallery.php  privacy.php  404.php    pages
...
media/about/                    timeline photos, generated by tools/build-about.py
...
**The /about timeline** is the `$timeline` array at the top of `about.php`, rendered as a pure-CSS timeline (`.timeline` in `styles.css`, no JS beyond the shared `.reveal`). Photos come from `assets/about/` via `tools/build-about.py`, which prints the `w`/`h` the array needs. The business JSON-LD deliberately has **no `foundingDate: 1974`** — the current business was founded in 2025; 1974 is the family firm it descends from. The 2002 photo shows a *telescopic* crane, not the knuckle-boom cranes the 2002 text describes; its alt text says so.
...
- **Two unverified claims** in the copy: 32 m platform height, 7 t forklift capacity. Neither confirmed. ("50 years" is now backed by the client-supplied 1974 founding date on `/about`.) An inflated lifting capacity carries liability beyond marketing — confirm before launch.
